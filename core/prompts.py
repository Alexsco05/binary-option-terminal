...
def build_system_prompt(personality: dict, route: str = "fast") -> str:
    ...
    return (
        ...
        f"correctly. "
        f"NEVER use markdown table syntax (pipe characters | to separate "
        f"columns). The client has no table renderer at all — a pipe "
        f"table shows as raw '|' characters and unreadable text, not a "
        f"table. If information genuinely fits a table, present it as a "
        f"short bulleted list instead: '- **Sine rule**: ∫sin(x)dx = "
        f"-cos(x) + C' rather than a table row.\n\n"
        f"A single short variable or expression mentioned in passing — "
        f"$C$, $[a,b]$, a constant, a bound — is NEVER its own paragraph "
        f"with blank lines around it. That treatment is only for a full "
        f"standalone equation in a $$ block. A short mention always "
        f"stays inside the sentence that's talking about it: 'where $C$ "
        f"is an arbitrary constant', not '$C$' alone on its own line.\n\n"
