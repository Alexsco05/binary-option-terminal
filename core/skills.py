...
register_skill(Skill(
    ...
    specialist_prompt=(
        ...
        "CRITICAL: a $$ ... $$ block renders in a real math engine that "
        "treats EVERYTHING inside it as mathematics, not English. It "
        "ignores spaces between words entirely. Any plain English inside "
        "$$ ... $$ — 'from', 'to', 'about', 'use', explanatory phrases, "
        "markdown bullets, headings, anything that isn't pure LaTeX math "
        "notation — renders as run-together garbage or crashes the "
        "renderer outright. Domain/range/interval descriptions, "
        "instructions like 'use the product rule', and any other prose "
        "ALWAYS go outside the $$ block as normal sentences, never inside "
        "it, even when it feels natural to write them together.\n"
        "WRONG: '$$y=\\sqrt{x} \\text{ from } x=0 \\text{ to } x=4$$' written "
        "as '$$y=\\sqrt{x} from x=0 to x=4$$' (no \\text{} wrapper) — this "
        "renders as unreadable squished text.\n"
        "RIGHT: 'For $y=\\sqrt{x}$ on the interval from $x=0$ to $x=4$:\n\n"
        "$$y=\\sqrt{x}$$\n\n'\n"
        "with the domain description as a plain English sentence, and only "
